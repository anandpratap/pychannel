...
class StressOmegaOmegaEquation(LaminarEquation):
    ...
    def calc_R11_residual(self, q):
        ...
        R11y = diff(y, R11)
        R11yy = diff2(y, R11)
        R = -self.P11 + self.eps_hat - self.PI11 + R11yy*(self.nu + self.nut*sigma_star) + sigma_star*self.nuty*R11y
        R[0] = -R11[0]
    ...
    def calc_R22_residual(self, q):
        ...
        R22y = diff(y, R22)
        R22yy = diff2(y, R22)
        R = -self.P22 + self.eps_hat - self.PI22 + R22yy*(self.nu + self.nut*sigma_star) + sigma_star*self.nuty*R22y
        R[0] = -R22[0]
